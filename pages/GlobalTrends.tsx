...
const GlobalTrends: React.FC<GlobalTrendsProps> = ({ user, isPaid, reviews, onSignInClick }) => {
      ...
      <div className="bg-navy min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="max-w-xl w-full bg-white/5 border border-white/10 backdrop-blur-3xl rounded-card p-10 md:p-16 text-center space-y-10 relative z-10 shadow-2xl">
          <div className="w-20 h-20 bg-accent text-white rounded-card flex items-center justify-center mx-auto shadow-2xl">
            <Icon name="fa-chart-pie" size={30} />
          </div>
          <div className="space-y-4">
            <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight uppercase tracking-widest">Global Analytics</h1>
  ...
  return (
    <div className="pb-20 bg-slate-100 min-h-screen">
      <section className="bg-navy text-white pt-20 md:pt-24 pb-24 md:pb-32 px-4 sm:px-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/2 h-full bg-accent/10 blur-[120px] rounded-full -mr-32 -mt-32"></div>
        <div className="container mx-auto relative z-10">
          <div className="max-w-4xl">
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6 leading-[1.1]">
              Global <span className="text-accent-soft">Sales Analytics</span>
            </h1>
      ...
      <div className="container mx-auto px-4 sm:px-6 -mt-10 relative z-20">
        <div className="bg-white p-6 md:p-8 rounded-card border border-slate-200 shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex flex-wrap gap-6 items-center">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Sector Benchmark</label>
            <select value={filterIndustry} onChange={(e) => setFilterIndustry(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 focus:border-accent/30 focus:bg-white rounded-xl px-5 py-4 text-sm font-bold text-slate-700 outline-none transition-all cursor-pointer">
              <option value="all">All Industries</option>
              {industries.map(i => <option key={i} value={i}>{i}</option>)}
            </select>
          </div>
          <div className="flex-1 min-w-[200px]">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Department Persona</label>
            <select value={filterTeam} onChange={(e) => setFilterTeam(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-100 focus:border-accent/30 focus:bg-white rounded-xl px-5 py-4 text-sm font-bold text-slate-700 outline-none transition-all cursor-pointer">
      ...
      <div className="container mx-auto px-4 sm:px-6 mt-12 md:mt-20">
        {!isPaid ? (
          <div className="bg-white rounded-card border-2 border-slate-200 shadow-xl overflow-hidden relative min-h-[600px]">
            <div className="absolute inset-0 bg-white/80 backdrop-blur-xl z-30 flex flex-col items-center justify-center p-10 text-center">
                <div className="w-20 h-20 bg-navy text-white rounded-card flex items-center justify-center mb-8 shadow-2xl">
                  <Icon name="fa-crown" size={30} />
                </div>
                <h3 className="text-3xl font-bold text-slate-900 mb-4 tracking-tight">Sales Pro Analytics Required</h3>
                <p className="text-slate-500 text-base max-w-md mx-auto mb-10 font-medium leading-relaxed">
                  Deep-dive friction maps, velocity benchmarks, and sector indices are reserved for Sales Pro members.
                </p>
                <Link to="/pricing" className="bg-accent text-white px-12 py-5 rounded-control font-bold uppercase tracking-widest shadow-2xl hover:bg-accent-700 transition-all">
                  Upgrade to Sales Pro
                </Link>
            </div>
            <div className="p-12 opacity-5 blur-2xl pointer-events-none select-none grayscale">
              <div className="grid grid-cols-4 gap-8 mb-12">
                <StatSummaryCard label="Reports" value="500" icon="fa-fingerprint" />
                <StatSummaryCard label="Win Rate" value="65%" icon="fa-trophy" />
                <StatSummaryCard label="Friction" value="3.2" icon="fa-handshake" />
                <StatSummaryCard label="Markets" value="20" icon="fa-globe" />
              </div>
              <div className="bg-slate-200 h-96 rounded-card"></div>
            </div>
          </div>
        ) : (
          <div className="space-y-12 md:space-y-16">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
              <div className="bg-navy p-10 md:p-12 rounded-card text-white shadow-2xl space-y-10 relative overflow-hidden border border-white/5">
                  ...
                  {stats.departments.length > 0 ? stats.departments.slice(0, 6).map((dept) => (
                    <div key={dept.name} className="space-y-2.5">
                      <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest text-slate-300">
                        <span>{dept.name}</span>
                        <span className="text-accent-soft">{dept.avgFriction}/5.0</span>
                      </div>
                      <div className="h-2 bg-slate-800/50 rounded-full overflow-hidden border border-white/5">
                        <div className="h-full bg-gradient-to-r from-accent to-accent-soft transition-all duration-1000" style={{ width: `${(parseFloat(dept.avgFriction) / 5) * 100}%` }}></div>
                      </div>
                    </div>
                  )) : (
              ...
              <div className="lg:col-span-2 bg-white rounded-card border border-slate-200 p-8 md:p-12 shadow-[0_8px_30px_rgba(0,0,0,0.04)] overflow-hidden">
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Industry Index</h3>
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-10">Closing Metrics by Sector</p>
                <div className="overflow-x-auto -mx-8 px-8">
                  <table className="w-full text-left min-w-[500px]">
                    <thead>
                      <tr className="border-b border-slate-100">
                        <th className="pb-6 text-[10px] font-bold uppercase text-slate-400 tracking-widest">Sector</th>
                        <th className="pb-6 text-[10px] font-bold uppercase text-slate-400 text-center tracking-widest">Win Rate</th>
                        <th className="pb-6 text-[10px] font-bold uppercase text-slate-400 text-right tracking-widest">TK Index</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {stats.topIndustries.map((ind) => (
                        <tr key={ind.name} className="group hover:bg-slate-50/50 transition-colors">
                          <td className="py-6">
                            <div className="font-bold text-slate-800 text-base group-hover:text-accent transition-colors">{ind.name}</div>
                            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">{ind.count} Reports Analysed</div>
                          </td>
                          <td className="py-6 text-center">
                            <div className="text-base font-bold text-slate-900">{ind.winRate}%</div>
                          </td>
                          <td className="py-6 text-right">
                            <div className="flex flex-col items-end">
                                <span className={`text-[11px] font-bold uppercase tracking-tight ${parseInt(ind.tireKickerScore.toString()) > 50 ? 'text-rose-500' : 'text-emerald-500'}`}>
            ...
            <div className="bg-white rounded-card border border-slate-200 p-10 md:p-14 shadow-[0_8px_30px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12">
                <div>
                    <h3 className="text-3xl font-bold text-slate-900 leading-tight">Velocity Matrix</h3>
                ...
                <div className="bg-accent-50 text-accent px-5 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest border border-accent/30">
                    Heatmap View
                </div>
              </div>
...
const ValueVelocityMatrix: React.FC<{ data: any }> = ({ data }) => {
  return (
    <table className="w-full border-collapse min-w-[900px]">
      <thead>
        <tr>
          <th className="p-5 text-[10px] font-bold uppercase text-slate-400 border-b border-slate-100 text-left">Closing Cycle</th>
          {TCV_ORDER.map(tcv => (
            <th key={tcv} className="p-5 text-[10px] font-bold uppercase text-slate-400 border-b border-slate-100 text-center tracking-tighter">
              {tcv}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {DURATION_ORDER.map(duration => (
          <tr key={duration} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors">
            <td className="p-5 text-[11px] font-bold text-slate-500 uppercase w-48">{duration}</td>
            {TCV_ORDER.map(tcv => {
              ...
              return (
                <td key={tcv} className="p-1">
                  <div className={`m-1 rounded-2xl py-5 text-center border transition-all ${
                    cell.count > 0
                    ? 'bg-accent text-white border-accent-700 shadow-lg scale-105 z-10'
                    : 'bg-slate-50/50 border-transparent text-slate-300 opacity-50'
                  }`}>
                    <div className="text-sm font-bold">{cell.count > 0 ? cell.count : '0'}</div>
                  </div>
                </td>
              );
            })}
...
const StatSummaryCard: React.FC<{ label: string, value: string | number, icon: string, color?: string }> = ({ label, value, icon, color = "text-slate-900" }) => (
  <div className="bg-white p-8 md:p-12 rounded-card border border-slate-200 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.04)] flex flex-col items-center justify-center text-center space-y-4 hover:shadow-xl hover:-translate-y-2 transition-all duration-300">
    <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-control bg-slate-50 border border-slate-100 flex items-center justify-center ${color}`}>
      <Icon name={icon} size={24} />
    </div>
    <div>
      <div className="text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{label}</div>
      <div className={`text-2xl md:text-5xl font-bold tracking-tight ${color}`}>{value}</div>
    </div>
  </div>
);
