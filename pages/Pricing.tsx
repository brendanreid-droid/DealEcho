...
const Pricing: React.FC<PricingProps> = ({ user, isPaid }) => {
    ...
    if (params.get('checkout') === 'success') {
      ...
      const poll = async () => {
        ...
        if (appAuth.currentUser) {
          ...
          if (tokenResult.claims.role === 'paid' || tokenResult.claims.role === 'admin') {
            setMessage({ text: 'Upgrade successful! Welcome to Sales Pro. 🎉', type: 'success' });
            return;
          }
        }
        if (attempts < maxAttempts) {
          ...
          setMessage({ text: 'Payment received! Your membership will activate shortly. Please refresh if needed.', type: 'success' });
        }
      };
      // Start polling after a short initial delay to give webhook time
      setTimeout(poll, 2000);
    } else if (params.get('checkout') === 'cancelled') {
      setMessage({ text: 'Checkout cancelled. No charges were made.', type: 'info' });
      navigate('/pricing', { replace: true });
    }
  ...
  const handleSubscribe = async () => {
    ...
    try {
      const createSession = httpsCallable<{ plan: 'monthly' | 'annual' }, { sessionUrl: string }>(
        functions,
      ...
      setMessage({ text: err.message || 'Failed to start checkout. Please try again.', type: 'error' });
  ...
  return (
    <div className="py-20 px-6 max-w-7xl mx-auto">
      {/* Toast-style notice */}
      {message && (
        <div className={`fixed top-24 right-6 z-50 px-6 py-4 rounded-2xl shadow-2xl font-bold text-sm flex items-center gap-3 animate-fade-in ${
          message.type === 'success' ? 'bg-emerald-600 text-white' : 
          message.type === 'error' ? 'bg-rose-600 text-white' : 'bg-slate-900 text-white'
        }`}>
          <i className={`fas ${message.type === 'success' ? 'fa-check-circle' : 'fa-info-circle'}`}></i>
          {message.text}
          <button onClick={() => setMessage(null)} className="ml-2 hover:opacity-60 transition-opacity">
      ...
      <div className="text-center mb-16 space-y-4">
        <h1 className="text-4xl md:text-6xl font-black text-[#1e293b] tracking-tight">Invest in your <span className="text-[#4f46e5]">Closing Rate</span>.</h1>
        ...
        <div className="flex items-center justify-center space-x-4 pt-8">
          ...
          >
            <div className={`w-6 h-6 bg-indigo-600 rounded-full transition-all transform ${isAnnual ? 'translate-x-6' : 'translate-x-0'}`}></div>
          </button>
          <span className={`text-sm font-bold ${isAnnual ? 'text-slate-900' : 'text-slate-400'}`}>Annual <span className="text-emerald-500 text-[10px] ml-1">Save 20%</span></span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        ...
        <div className="bg-white border-2 border-slate-50 p-10 rounded-[40px] flex flex-col h-full hover:border-slate-100 transition-all">
          <div className="mb-8">
            ...
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">Free Forever</p>
          </div>
          
          <ul className="space-y-4 mb-10 flex-grow">
            {['Search Companies', 'Basic Review Feeds', 'Submit Intelligence', '3 Tracked Accounts'].map((item) => (
        ...
        <div className="bg-[#101426] p-10 rounded-[40px] flex flex-col h-full text-white relative overflow-hidden shadow-2xl shadow-indigo-100">
          <div className="absolute top-0 right-0 bg-indigo-600 text-[10px] font-black uppercase tracking-widest px-6 py-2 rounded-bl-2xl">Recommended</div>
          
          <div className="mb-8">
            <h3 className="text-xl font-black mb-2">Sales Pro Intel</h3>
            <div className="flex items-baseline">
              <span className="text-4xl font-black">A${isAnnual ? '144' : '15'}</span>
            ...
            <p className="text-indigo-400 text-xs font-bold uppercase tracking-widest mt-2">
              {isAnnual ? 'Billed Annually ($12/mo) (AUD)' : 'Billed Monthly (AUD)'}
            </p>
          </div>
          ...
          <ul className="space-y-4 mb-10 flex-grow">
            {[
              ...
              <li key={item} className="flex items-center text-slate-300 text-sm font-medium">
          ...
          {isPaid ? (
            ...
            >
              {isProcessing ? <i className="fas fa-spinner fa-spin"></i> : 'Upgrade to Pro'}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mt-20 bg-slate-50 p-12 rounded-[48px] text-center border border-slate-100">
        <h4 className="text-xl font-bold text-slate-900 mb-4">The Enterprise Choice</h4>
        <p className="text-slate-500 max-w-xl mx-auto text-sm leading-relaxed mb-8">
          Need a license for your whole sales team? Get centralized billing, admin controls, and private data silos.
        </p>
        <button className="text-indigo-600 font-black text-[11px] uppercase tracking-[0.2em] border-b-2 border-indigo-100 hover:border-indigo-600 transition-all">Contact Sales for Enterprise</button>
      </div>
    </div>
  );
};
