...
import {
  ...
  mergeFlags,
  rank,
  // AccountFlag comes from the AI half, which already lived server-side.
  evidenceBar,
  capSeverity,
  groupFlags,
  MAX_RISK_FLAGS,
...
describe("redactFlag", () => {
  const flag: AccountFlag = {
    ...
    severity: "critical",
    stat: "4 of 6 deals",
    qualify: ["who signs off", "how long it took last time"],
  ...
  it("removes the evidence, the qualification points and the citations", () => {
    const r = redactFlag(flag);
    expect(r.stat).toBe("");
    expect(r.qualify).toEqual([]);
    expect(r.reviewIds).toEqual([]);
  });

  it("keeps what a free caller is allowed to see", () => {
    const r = redactFlag(flag);
    expect(r.label).toBe("Security review is a gate");
    expect(r.severity).toBe("critical");
    expect(r.polarity).toBe("risk");
  });

  it("leaves the original untouched", () => {
    redactFlag(flag);
    expect(flag.stat).toBe("4 of 6 deals");
    expect(flag.reviewIds).toHaveLength(4);
  });

  it("serialises with no paid field surviving anywhere in the payload", () => {
    // The point of redacting server-side: nothing to recover from the wire.
    const json = JSON.stringify(redactFlag(flag));
    expect(json).not.toContain("4 of 6 deals");
    expect(json).not.toContain("who signs off");
