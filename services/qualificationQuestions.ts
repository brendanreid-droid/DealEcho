...
export interface QualificationQuestion {
  ...
  /** Higher sorts first. */
  priority: number;
  /** Observed rate for this rule's trigger, 0-1. Blended into ranking. */
  strength: number;
}
 ...
 * (legacy v1 reviews answer none of them), so a 9-review account can still have
 * a 1-of-1 modal. The friction rules already floor at 2 via `friction(...)`.
 */
...
/** Category weight plus up to 20 points for how strongly the account shows it. */
export function rank(q: QualificationQuestion): number {
  return q.priority + q.strength * 20;
}
...
export const RULES: Rule[] = [
  ...
  },
  {
    build: (m) => {
      ...
      return {
        ...
        question:
          "What are the written success criteria for the pilot, who signs off that they were met, and what is the next step once they are?",
  ...
  },
  {
    build: (m) => {
      ...
      if (!s || s.total < MIN_RULE_SAMPLE || (s.value !== "1-3 Months" && s.value !== "3+ Months")) return null;
      return {
        id: "verbal-drift",
        question:
  ...
  },
  {
    build: (m) => {
      ...
      if (!s || s.total < MIN_RULE_SAMPLE || !["Net 60", "Net 90", "Net 120+"].includes(s.value)) return null;
      return {
        id: "payment-terms",
        question: `Your standard terms appear to be ${s.value} - what would justify an exception, and who approves it?`,
