    ...
    return (
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center space-x-3 md:space-x-4">
            <CompanyLogo
              ...
              size="lg"
              className="group-hover:scale-105 transition shadow-sm border border-slate-100"
            />
            <div>
              <h3 className="text-lg md:text-xl font-bold text-[#1e293b] leading-tight group-hover:text-indigo-600 transition-colors">
                {company.name}
              </h3>
          ...
          <div className="text-right flex flex-col items-end space-y-2 relative">
            ...
            >
              ...
              {isScoreHovered && (
                <div className="absolute top-full right-0 mt-4 w-72 bg-slate-900 text-white p-6 rounded-[28px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.4)] z-[110] animate-in fade-in slide-in-from-top-2 border border-white/10 text-left pointer-events-none">
                  <h4 className="text-[11px] font-black uppercase tracking-[0.15em] text-indigo-400 mb-4 border-b border-white/5 pb-3">
                    Buyer Intelligence Summary
                  </h4>
                  <p className="text-[10px] text-slate-300 leading-relaxed mb-5 font-medium">
                    Aggregate account health based on {company.reports} verified
                    deal mechanics:
                  </p>
            ...
            <button
              ...
              className={`p-2 rounded-xl border transition-all ${!user
                  ? "bg-slate-50 border-slate-200 text-slate-300 hover:text-indigo-500 hover:bg-white hover:border-indigo-200"
                  : isTracked
                    ? "bg-indigo-50 border-indigo-200 text-indigo-600"
                    : "bg-slate-50 border-slate-200 text-slate-400 hover:text-indigo-500 hover:bg-white hover:border-indigo-200"
                }`}
            >
              {/* If not logged in, show a lock icon to hint it's a gated feature */}
              <i
                className={`${!user ? "fas fa-lock" : isTracked ? "fas fa-bookmark" : "far fa-bookmark"} text-sm md:text-lg`}
              ></i>
            </button>
          </div>
        </div>
        ...
        <div className="relative mb-6 overflow-hidden rounded-xl">
          <div
            className={`text-slate-600 text-[13px] md:text-[14px] leading-relaxed line-clamp-2 transition-all duration-500 font-medium ${!isPro ? "filter blur-md opacity-20 pointer-events-none" : ""}`}
          >
            {company.desc}
          </div>
          {!isPro && (
        ...
        <div className="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
          <div className="flex -space-x-2">
            ...
            <div className="pl-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
