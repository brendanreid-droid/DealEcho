import { AccountFlag, FlagSeverity } from "./accountFlags";
import { DealMechanics, FrictionStat } from "./dealMechanics";

/**
 * Mirrors the structured half of services/accountFlags.ts - the deterministic
 * rule bank over schema v2 fields. The AI half already lives in accountFlags.ts
 * in this workspace; this is the other half, moved server-side so the extension
 * and the company profile cannot drift apart again.
 *
 * The rules are config, not logic. Port them verbatim; the ported tests are
...
/**
 * Labels, severity and polarity only.
 *
 * For callers who have not paid for the detail. Everything that carries the
 * finding itself - the evidence line, the qualification points, the backing
 * review IDs - is removed from the payload rather than hidden by the client,
 * because a client-side blur is one devtools tab away from being no paywall at
 * all. A free caller still learns how many flags exist and whether they are
 * risks or strengths, which is what makes the upgrade prompt honest.
 */
export function redactFlag(f: AccountFlag): AccountFlag {
  return { ...f, stat: "", qualify: [], reviewIds: [] };
}
